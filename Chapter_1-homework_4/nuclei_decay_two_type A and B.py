# -*- coding: utf-8 -*-
"""
Created on Mon Apr 06 00:56:06 2016
@author: luokaifa
"""
# Update: 20160403
...
#                     dn_A/dt = -n_A/tauA 
#                     dn_B/dt = n_A/tauA - n_B/tauB
#     where tauA and tauB is decay constant
...
# declare the variables & para.
N_A = []    
N_B = []    # N_A & N_B are number of two type nuclei
t = []      # time
tauA= 10    #where 10 can't be replaced by 10. ,else something shit will happen!!!
tauB= 0.
time = 0
n = 1
dt= 1        # tauA&tauB: decay constants, dt: numerical step size, time: total time, n:total number of steps   
...
times = int(raw_input('times of differnt dt (more than 1) :  '))
            # dt ranges
color = ['ob','xg','*y','oy','r','+m','xc']+['sy']*(times-7)
            # color sets of plot
...
def initialize(N_A,N_B,t,_tau,_tauB,_dt,_time,_n): 
    global tauA,tauB,dt,time,n
    ...
    tauB = float(raw_input('decay constant ratio tauA/tauB:    '))
    time = float(raw_input('numerical total time:  '))

# calculate the time dependence of N_A & N_B using Euler method
def calculate(N_A,N_B,t,tauA,tauB,dt,time,n):
    for i in range(n): 
        N_A.append(N_A[-1]-(N_A[-1])/tauA*dt)
        N_B.append(N_B[-1]+((N_A[-1])/tauA-(N_B[-1])/tauB)*dt)
        t.append(t[-1]+dt)

# store data:
#      pickle data, stored in 'nuclei_decay_two_type_pickle.txt'
#      txt data, stored in 'nuclei_decay_two_type_txt.txt'
def store(N_A,N_B,t,tauA,tauB,dt,time,n):
...
initialize(N_A,N_B,t,tauA,tauB,dt,time,n)
c1 = N_A[0]/1.0
c2 = (N_B[0]-N_A[0]/(1/tauA+1/tauB))/1.0
for i in range(times): 
    delete()
    dt = float(raw_input('numerical step size:  '))
    n = int((time/dt))
    calculate(N_A,N_B,t,tauA,tauB,dt,time,n)
    store(N_A,N_B,t,tauA,tauB,dt,time,n)
    N_A_total.append(N_A)
    N_B_total.append(N_B)
    t_total.append(t)
    dt_total.append(dt)
    N_A_theory = c1*np.exp(-1/tauA*(array(t)-t[0]))+N_A
    N_B_theory = c1*tauB/(tauA-tauB)*np.exp((-1/tauA)*(array(t)-t[0]))+c2*np.exp(-1/tauB*(array(t)-t[0]))+N_B
...
fig_all=figure(figsize=(14,10),dpi=80)
...
xlabel('time')
ylabel('difference')
title('errors between theory and simulation',fontsize=20)
legend(loc ='best')

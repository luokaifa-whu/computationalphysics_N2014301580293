...
从以上各图可以看出，物理摆的周期是逐渐减小的，而且，数值模拟的结果显示角速度和最大幅角随着时间推移会逐渐增大，这与物理摆的机械能守恒相矛盾，相位图中也直观地反映出了这一点，经过分析认为是由于算法的问题导致模拟过程中系统的能量不守恒导致的错误。下面换另一种算法来进行模拟。<br/>
